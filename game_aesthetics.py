...
game_over_dialogue = f"""
    {BOLD}{RED}GAME OVER{RESET} — {BOLD}{ITALIC}Brewing Accident{RESET}

    As you drop the final wrong ingredient into the cauldron,
    the mixture twitches… bubbles… then starts glowing a very dangerous shade of purple.

    You blink.
    The cauldron growls.
    That is definitely not normal.

    Before you can step back, the potion erupts in a flash of smoke and sparks,
    filling the whole shop with a loud PFFFFFT—BOOM!

    The shelves rattle.
    The windows shake.
    And you, poor orphan young witch, get launched straight out of existence in a puff of glittery smoke.

    {ITALIC}The spirits whisper:
    {FAINT}    “A single wrong step can turn a potion into a disaster…”{RESET}

    And so your journey ends here.
    The shop falls silent once more.

    {RED}{UNDERLINED}GAME OVER{RESET}
...
def game_over():
    ...
    print(game_title)
    print(breaker)
    animate_string(game_over_dialogue, type_of_animation = 1, delay = 0.03)
    time.sleep(5)
